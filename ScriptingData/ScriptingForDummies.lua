...
--Example for a Q SkillShot* - You can search the skillshots abilities stats in a league champ Wikia. Some of them are in the bottom.
local ChampQ = {delay = 0.25, speed = 2000, width = 70, range = 500} 

--Main Menu Tab
local ChampMenu = Menu("KledNation", "KledNation")

--Menu Options / You need to put the "Champ" in "ChampMenu" the name you want, change it in everything.
  ChampMenu:Menu("Combo", "Combo")
  ChampMenu.Combo:Boolean("useQ", "Use Q", true) --Use Q
  ChampMenu.Combo:Boolean("useW", "Use W", true) --Use W
  ChampMenu.Combo:Boolean("useE", "Use E", true) -- Use E
  ChampMenu.Combo:Boolean("useE", "Use R", true) --Use R
  ChampMenu.Combo:Boolean("useTiamat", "Use Tiamat", true)
  ChampMenu.Combo:Boolean("useHydra", "Use Ravenous Hydra", true)
  ChampMenu.Combo:Boolean("useTitanic", "Use Titanic Hydra", true)
  ChampMenu.Combo:Boolean("useBOTRK", "Use BladeOfTheRuinedKing", true)
  ChampMenu.Combo:Boolean("useCut", "Use Bilgewater Cutlass", true)  
  ChampMenu.Combo:Boolean("useYoum", "Use Youmuu's Ghostblade", true)
  
--ComboTables=Items/Spells
OnTick(function()
--If the Orbwalker is in the IOW mode the combo will work.
...
--Combo        
      local target = GetCurrentTarget() 
        ...
        if ValidTarget(target,GetRange(myHero) + GetHitBox(target)) and CanUseSpell(myHero,_W) == READY and ChampMenu.Combo.useW:Value() then
          CastSpell(_W)  --Casting R as a Spell
        end 
        if GetItemSlot(myHero, 3077) > 0 and IsReady(GetItemSlot(myHero, 3077)) and ChampMenu.Combo.useTiamat:Value() then
          CastSpell(GetItemSlot(myHero, 3077))  --Tiamat
        end
        if GetItemSlot(myHero, 3074) > 0 and IsReady(GetItemSlot(myHero, 3074)) and ChampMenu.Combo.useHydra:Value() then
          CastSpell(GetItemSlot(myHero, 3074))  --Raveneous Hydra
        end
        if GetItemSlot(myHero, 3748) > 0 and IsReady(GetItemSlot(myHero, 3748)) and ChampMenu.Combo.useTitanic:Value() then
          CastSpell(GetItemSlot(myHero, 3748))  --Titanic Hydra
        end 
        if GetItemSlot(myHero, 3153) > 0 and IsReady(GetItemSlot(myHero, 3153)) and ChampMenu.Combo.useBOTRK:Value() then
          CastTargetSpell(target, GetItemSlot(myHero, 3153))  --Blade Of The Ruined King
        end        
        if GetItemSlot(myHero, 3144) > 0 and IsReady(GetItemSlot(myHero, 3144)) and ChampMenu.Combo.useCut:Value() then
...
--SkinChanger Example
ChampMenu:SubMenu("SkinChanger", "SkinChanger")

--You can get all the Skins datas from this link from Scortch : https://github.com/Farscape2000/GOS/blob/master/Better%20SkinChanger.lua
local skinMeta = {["Champ"] = {"SkinName1", "SkinName2"}},
ChampMenu.SkinChanger:DropDown('skin', myHero.charName.. " Skins", 1, skinMeta[myHero.charName], HeroSkinChanger, true)
ChampMenu.SkinChanger.skin.callback = function(model) HeroSkinChanger(myHero, model - 1) print(skinMeta[myHero.charName][model] .." ".. myHero.charName .. " Loaded!") end

--SpellDataBase
--Original Link from KeVuong : https://github.com/KeVuong/GoS/blob/master/Data/SpellDatabase.lua
--Outdated / Doesn't have Kled or Ivern : Use this on the "local ChampQ" in line 12.
